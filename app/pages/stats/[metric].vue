...
/** Services */
import { getStartChainDate } from "~/services/config.js"
import { exportSVGToPNG, exportToCSV } from "~/services/utils/export.js"
import { abbreviate, capitilize, capitalizeAndReplace, isMobile } from "~/services/utils/index.js"

/** API */
import { fetchSeries, fetchRollupsSeries, fetchSeriesCumulative, fetchTVL, fetchTVS } from "~/services/api/stats.js"
...
const handleOpenChartModal = () => {
	...
	cacheStore.chart.view =
		series.value.page === "tvs" ? "barplot-stacked" : series.value.page === "networks" ? "barplot-stacked-rollups" : chartView.value
		...
		<Flex direction="column" gap="16">
			...
			<Flex v-if="series?.name" align="center" justify="between" wide :class="$style.header">
				<Flex align="center" gap="8">
					...
					<Text v-if="totalValue" size="20" weight="600" color="brand" justify="start">
		...
		<Flex v-if="series?.name === 'tvs'" :class="$style.section">
			...
			<Flex :class="[$style.section, $style.section_content]">
				<Text :class="$style.section_paragraph"
					...
					that posts data to Celestia. We consider a network “secured by Celestia” if it uses Celestia as its data availability
					(DA) layer, regardless of its execution or settlement environment.</Text
				>

				<Text :class="$style.section_paragraph"
					>To determine each network’s TVL, we fetch data from one of the following public sources:</Text
				>
				<ul style="margin-top: -8px">
					<li>
						<a href="https://l2beat.com" target="_blank" :class="$style.link">L2BEAT</a> - includes canonically and externally
						bridged assets, as well as natively minted tokens;
					</li>
					<li>
						<a href="https://defillama.com" target="_blank" :class="$style.link">DeFiLlama</a> - focuses on assets actively
						engaged in dApps on the network.
					</li>
				</ul>

				<Text :class="$style.section_paragraph"
					>In addition to the network TVLs, we also include the circulating supply of the Celestia (TIA) token to reflect the
					value secured by the base layer itself. This gives a more holistic view of the total value secured by the Celestia
					network.</Text
				>
			</Flex>
		</Flex>
