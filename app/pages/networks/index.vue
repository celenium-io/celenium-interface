...
import { getLastActivityCategory, getRankCategory } from "~/services/constants/rollups.js"
import { rollupRankingServiceURL } from "~/services/config.js"

/** API */
import { fetchRollups, fetchRollupsRanking } from "~/services/api/rollup.js"
...
const providers = computed(() => [...new Set(rollups.value?.map((r) => r.provider))].filter(Boolean).sort((a, b) => a.localeCompare(b)))
			...
			<Flex align="center" justify="between" wrap="wrap" gap="8" :class="$style.settings">
				<Flex wrap="wrap" align="center" gap="8">
						...
						<template #content>
							<Flex direction="column" gap="12">
								<Text size="12" weight="600" color="secondary">
									{{ `Filter by ${p === "activity_rank" ? "Rank" : capitilize(keyMap[p])}` }}
								</Text>
					...
					<Button @click="showInactive = !showInactive" type="secondary" size="mini">
						<Icon :name="showInactive ? 'eye' : 'eye-off'" size="12" :color="showInactive ? 'brand' : 'secondary'" />
