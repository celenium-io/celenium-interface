...
/** Services */
import { roundTo, sortArrayOfObjects } from "~/services/utils/index.js"

/** API */
import { fetchValidators, fetchValidatorsMetrics, fetchValidatorMetrics } from "~/services/api/validator.js"
...
const getValidators = async () => {
	...
	if (data.value) {
		validators.value = sortArrayOfObjects(
			data.value.filter((v) => v.id !== props.validator.id),
			"moniker",
		)
	}
}
...
<template>
	<Flex direction="column" justify="between" align="center" gap="12" wide :class="$style.wrapper">
							...
							<template v-if="filteredValidators.length">
								...
								>
									...
									<Text
										size="12"
										color="primary"
										:weight="selectedItem.moniker === v.moniker ? '600' : '500'"
										:class="$style.title"
									>
		...
		<RadarChart :series="metricsSeries" :isLoading="isLoading" :class="$style.chart" />
	</Flex>
</template>
