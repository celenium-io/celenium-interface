				...
				<Flex align="center" justify="between">
					...
					<Tooltip>
						<Text size="12" weight="600" color="secondary">
							{{
								status === STATUS_MAP.Arriving
									? "soon"
									: DateTime.now().plus({ seconds: secondsToSelectedBlock }).setLocale("en").toRelative()
							}}
						</Text>
							...
							<Text size="12" weight="600" color="secondary">
								{{
									status === STATUS_MAP.Arriving
										? "soon"
										: DateTime.now()
												.plus({ seconds: secondsToSelectedBlock })
												.setLocale("en")
												.toFormat("TT dd LLL yyyy")
								}}
							</Text>
						</template>
					</Tooltip>
