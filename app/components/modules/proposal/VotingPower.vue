...
const voteDistribution = computed(() => {
	...
	Object.keys(voteKinds).forEach((kind) => {
		...
		const shareOfVotes = roundTo(
			Number((props.proposal[`${kind}_voting_power`] / 1_000_000) * 100 || 0) / (props.proposal.voting_power / 1_000_000),
			0,
		)
		distribution[kind].shareOfVotes =
			shareOfVotes === 0 && props.proposal[kind]
				? "< 1%"
		...
		<Flex v-if="expand" direction="column" gap="16">
			<Flex v-for="vote in votes" align="center" justify="between">
				...
				<Text
					size="12"
					weight="600"
					:color="!proposal[vote] || voteDistribution[vote].shareOfVotes === '< 1%' ? 'tertiary' : 'secondary'"
				>
