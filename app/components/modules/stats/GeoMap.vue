...
/** Services */
import { isMobile, sortArrayOfObjects } from "~/services/utils/index.js"

/** Constants */
import { convertCountryCode, getCountryByCity, getCountryCentroid, getRandomLoaderPath } from "~/services/constants/stats.js"

/** API */
import { fetchNodeStats } from "~/services/api/stats.js"
...
const buildChart = async (chart) => {
	...
	const mousemove = function (event, d) {
		...
		if (chartView.value === "countries") {
			...
			let tooltipContent = sortArrayOfObjects(hoverCities, "amount", false)
				.map((c) => {
					return `
                    <div class="flex items-center gap--8">
	...
	if (chartView.value === "countries") {
		...
		map.transition()
			.duration(700)
			.attr("fill", (d) => (chartView.value === "countries" ? (d.amount ? countryColor(+d.amount) : "transparent") : "transparent"))
	} else if (chartView.value === "cities") {
	...
	if (!isMobile()) {
		if (chartView.value === "countries" && countryMaxAmount) {
			...
			const legendScale = d3.scaleLinear().domain([1, countryMaxAmount]).range([1, legend.width])

			const legendAxis = d3.axisBottom(legendScale).tickValues(legendValues).tickFormat(d3.format("d"))
			...
			legendGroup.transition().duration(1_000).style("opacity", 1)
		} else if (chartView.value === "cities") {
			legendValues = [1, 25, 100]
			legendCitiesMarkers = svg
				.append("g")
				...
				.style("fill", "var(--brand)")
				.attr("fill-opacity", 0)
			legendCitiesMarkers.transition().duration(1_000).attr("fill-opacity", 0.7)

			legendCitiesLabels = svg
				.append("g")
				...
				.attr("fill-opacity", 0)
			legendCitiesLabels.transition().duration(1_000).attr("fill-opacity", 1)
		}
	}
	...
	const zoom = d3
		...
		.on("zoom", (event) => {
			...
			if (chartView.value === "countries" && legendValues.length) {
				if (zoomScale > 3) {
					legendGroup.transition().duration(1_000).style("opacity", 0)
				} else {
					legendGroup.transition().duration(1_000).style("opacity", 1)
				}
			} else if (chartView.value === "cities") {
				if (zoomScale > 1) {
					...
					if (legendValues.length) {
						if (zoomScale > 3) {
							legendCitiesMarkers.transition().duration(1_000).attr("fill-opacity", 0)
							legendCitiesLabels.transition().duration(1_000).attr("fill-opacity", 0)
						} else {
							legendCitiesMarkers.transition().duration(1_000).attr("fill-opacity", 0.7)
							legendCitiesLabels.transition().duration(1_000).attr("fill-opacity", 1)
						}
...
onMounted(async () => {
	...
	try {
		...
		if (geoData?.features.length) {
			...
			nodeCityData.value = Object.values(
				cityData.reduce((acc, obj) => {
					if (!acc[obj.name]) {
						acc[obj.name] = { ...obj }
					} else {
