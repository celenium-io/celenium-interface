...
/** Services */
import { abbreviate, comma, formatBytes, sortArrayOfObjects } from "~/services/utils/index.js"
...
const buildChart = (chart, data) => {
	...
	// X scale
	const x = d3.scalePoint().range([0, width]).padding(1).domain(props.metrics)
	...
	svg.selectAll("middle-axis")
		...
		.each(function (d, i) {
			...
			g.call(d3.axisLeft().scale(y).ticks(0).tickSizeOuter(0))

			// Outer ticks
			g.append("line").attr("x1", -10).attr("y1", margin.top).attr("x2", 10).attr("y2", margin.top)
			g.append("line")
				.attr("x1", -10)
				.attr("y1", height - 2)
				.attr("x2", 10)
	...
	svg.selectAll("line, path:not(.graph)").style("stroke", "#6d696b").style("stroke-width", "2.5px")

	edgeAxis.forEach((m, i) => {
		svg.selectAll("labels")
			.data(sortedData.value[m])
			.enter()
			.append("text")
					...
					<table>
						...
						<tbody>
							<tr v-for="r in selectedRollupsData">
								...
								<td>
									<Icon
										@click.prevent.stop="selectRollup(r.slug)"
										name="close"
										size="16"
										color="secondary"
										:class="$style.remove_icon"
									/>
								</td>
							</tr>
						</tbody>
