...
/** API */
import { fetchIbcTransfers } from "~/services/api/ibc.js"
...
const getTransfers = async () => {
	...
	const { data } = await useAsyncData(`latest-ibc-transfers`, () =>
		fetchIbcTransfers({
			offset: 0,
			limit: 5,
			sort: "desc",
...
<template>
	<Flex wide direction="column" gap="4">
		<Flex align="center" gap="8" :class="$style.header">
			...
			<Text size="13" weight="600" color="primary">Latest IBC transfers</Text>
		</Flex>
		...
		<Flex direction="column" gap="16" :class="$style.transfers_body">
			...
			<div :class="$style.bottom">
				<Button link="/ibc/transfers" type="secondary" size="small" wide>
