...
/** Constants */
import { IbcChainName } from "~/services/constants/ibc.js"
...
<template>
	<Flex align="center" gap="16" :class="$style.wrapper">
		...
		<Flex
			v-if="largestTransfer"
			@click="handleOpenTransferModal(largestTransfer)"
			wide
			direction="column"
			gap="12"
			:class="[$style.card, $style.hoverable]"
		>
			<Flex align="center" justify="between">
				<Flex align="center" gap="8">
		...
		<Flex wide direction="column" gap="12" :class="$style.card">
			...
			<Text size="15" weight="600" color="primary">
				...
				<Text size="12" color="tertiary">{{
					busiestChannel ? (IbcChainName[busiestChannel.chain_id] ?? "Unknown") : "Unknown"
				}}</Text>
			</Text>
