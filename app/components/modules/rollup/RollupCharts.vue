...
/** Services */
import { abbreviate, formatBytes, hexToRgba, sortArrayOfObjects, spaces, tia } from "~/services/utils/index.js"
import {
	getFormatKey,
	createDataMap,
	generateDateForPeriod,
	generateSeriesData,
	PERIODS as periods,
} from "~/services/utils/entityCharts.js"

/** API */
import { fetchRollupSeries } from "~/services/api/stats.js"
import { fetchRollups, fetchRollupTVL, fetchRollupUops } from "~/services/api/rollup.js"
		...
		<Flex direction="column">
			...
			<Flex justify="between" gap="32" :class="$style.data">
				...
				<Flex ref="comparisonChartEl" direction="column" gap="12" :style="{ minHeight: '207px' }">
					...
					<Flex
						v-if="comparisonData[0]?.size_graph || isLoadingComparisonData"
						direction="column"
						gap="12"
						:class="[$style.chart_wrapper_single, isLoadingComparisonData && $style.loading]"
					>
						<Flex direction="column" gap="12">
