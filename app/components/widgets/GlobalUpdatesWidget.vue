<script setup>
/** API */
import { fetchValidatorsUpgradeByVersion } from "~/services/api/validator.js"
...
/** Services */
import { capitilize, capitalizeAndReplace, comma, roundTo } from "~/services/utils/index.js"
		...
		<Flex @click="handleClick" direction="column" gap="8" :key="activeIndex" :class="$style.wrapper">
			<Flex justify="between">
				<Flex align="center" gap="6">
					<Icon :name="updateMeta?.icon" size="18" color="brand" />
				...
				<Flex v-if="updates?.length > 1" @click.stop="handleSwitchUpdate" align="center" justify="center" :class="$style.switcher">
					<Icon name="repeat" size="14" color="brand" style="margin-bottom: 4; margin-left: 4" />
				</Flex>
			</Flex>
				...
				<Flex direction="column" gap="6" wide>
					...
					<Flex align="center" gap="4" :class="$style.voting_wrapper">
						...
						<Tooltip
							...
							wide
							:trigger-width="`${Math.max(10, (showingUpdate.no_with_veto * 100) / showingUpdate.votes_count)}%`"
						>
							<div
								:style="{
									background: 'var(--red)',
			...
			<Flex v-else-if="showingUpdate?.kind === 'node_upgrade'" justify="between" direction="column" gap="12" style="flex: 1">
				<Flex direction="column" gap="16">
					<Flex align="center" gap="8" wide>
						<Text size="12" weight="500" color="tertiary">Status</Text>
						<Text v-if="showingUpdate?.status" size="12" weight="600" color="brand">
					...
					<Flex align="center" justify="between" wide>
						<Tooltip position="start">
							<Flex align="center" gap="6">
								...
								<Icon name="warning" size="12" color="secondary" />
							</Flex>
							...
							<template #content>
								<Flex wide>
									<Text
										size="12"
										color="secondary"
										style="width: 300px; line-height: 1.2; white-space: normal; overflow-wrap: anywhere"
									>
						...
						<Text size="12" weight="600" color="primary">
							<Text :color="showingUpdate?.votedShare > 83.3 ? 'brand' : 'tertiary'">
