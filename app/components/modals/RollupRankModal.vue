...
/** Services */
import { roundTo } from "~/services/utils/index.js"
	...
	<Modal :show="show" @onClose="emit('onClose')" width="600" disable-trap>
		<Flex direction="column" gap="24" wide>
			...
			<Flex direction="column" gap="16" wide :class="$style.wrapper">
				...
				<Text size="12" color="secondary" :style="{ lineHeight: '1.4' }">
					We use the following metric categories: Quantitative, Reciprocal quantitative, Time-Based, Logarithmic and Custom (for
					TVL rank calculation).
				</Text>

				<Text size="12" color="secondary" :style="{ lineHeight: '1.4' }">
					Learn more about rank calculation in the
					<NuxtLink to="https://docs.celenium.io/features/rollup-activity-rank" target="_blank" :class="$style.link"
						>overview article</NuxtLink
					>
					and the
					<NuxtLink to="https://docs.celenium.io/features/rollup-activity-rank-specification" target="_blank" :class="$style.link"
						>detailed specification</NuxtLink
					>
					.
				</Text>
