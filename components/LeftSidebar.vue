...
<template>
	<Flex direction="column" gap="32" justify="between" :class="[$style.wrapper, appStore.showSidebar && $style.show]">
		<Flex direction="column" gap="32" :class="$style.content">
			<Flex justify="between" align="center">
				<NuxtLink to="/" :class="$style.logo">
					<Flex align="center" gap="8">
						<Icon v-if="getNetworkName() !== 'Mammoth'" name="logo" size="16" color="tertiary"
							  :class="$style.logo_symbol" />
						<Text v-else size="16" style="filter: grayscale(1)">🦣</Text>

						<svg width="86" height="14" viewBox="0 0 96 16" xmlns="http://www.w3.org/2000/svg"
			...
			<Flex direction="column" gap="2">
				<Flex @click="isModularLinksCollapsed = !isModularLinksCollapsed" align="center" gap="4"
					  :class="$style.group_title">
				...
				<Flex v-if="!isModularLinksCollapsed" direction="column" gap="2">
					<NavLink v-for="link in modularLinks.filter((l) => !l.hide)" :link="link"
							 @onClose="handleOnClose" />
				</Flex>
			</Flex>

			<Flex direction="column" gap="2">
				<Flex @click="isToolsLinkCollapsed = !isToolsLinkCollapsed" align="center" gap="4"
					  :class="$style.group_title">
		...
		<Flex direction="column" gap="16" style="margin-right: 20px">
			<Flex v-if="settings.sidebar.footer.showLightNode"
				  @click="modalsStore.open('lightNode')"
				  align="center" gap="8" justify="between"
				  :class="$style.light_node_btn">
				<Flex align="center" gap="8">
					<Icon
					...
					<Text v-if="nodeStore.status === StatusMap.Started" size="13" weight="600" color="primary">Running
					</Text>
					<Text size="13" weight="600" color="secondary">Node</Text>
				</Flex>

				<Icon v-if="nodeStore.status !== StatusMap.Started" name="arrow-narrow-right" size="14"
					  color="secondary" />
				<Text v-else size="12" weight="600" color="tertiary">{{ nodeStore.percentage.toFixed(0) }}%</Text>
			</Flex>

			<Dropdown v-if="settings.sidebar.footer.showNetworkSelector" position="end" fullWidth
