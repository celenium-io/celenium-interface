...
const buildChart = (chart, data) => {
	...
	svg.append("g")
		...
		.call((g) => g.selectAll(".tick:not(:first-of-type) line").attr("stroke-opacity", 0.7).attr("stroke-dasharray", "10, 10"))
	...
	const legendValues = [500 * 1_024 * 1_024, midSize * 0.5, maxSize / 2]
	const xCircle = width - 50
	...
	svg.selectAll("legend")
		...
		.attr("x2", xLabel)
		.attr("y1", (d, i) => {
			return yCircle - size(d) + (i === 0 ? 5 : i === 2 ? -7 : 0)
		})
		.attr("y2", (d, i) => {
			return yCircle - size(d) + (i === 0 ? 5 : i === 2 ? -7 : 0)
		})
	...
	svg.selectAll("legend")
		...
		.attr("x", xLabel)
		.attr("y", (d, i) => {
			return yCircle - size(d) - 5 + (i === 0 ? 5 : i === 2 ? -7 : 0)
		})
		.text((d, i) => {
			return i === 0 ? `<${formatBytes(d, 0)}` : formatBytes(d, 0)
		})
		.style("font-size", 10)
